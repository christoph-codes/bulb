...
export const AuthProvider = ({ children }) => {
  ...
  const loginWithEmailAndPassword = (email, password) => {
    return new Promise((resolve, reject) => {
      if (!email || !password) {
        return reject({
          error: { message: "Email and password are required to login" },
        });
      }

      try {
        const response = BULB_API.post("/auth/login", { email, password });
        console.log("bulbapi response", response);
        if (!response) {
          return reject({
            error: { message: "These credentials are invalid" },
          });
        }
        if (response.error) {
      ...
      BULB_API.post("/createAccount", {
        ...
        .then((res) => {
          if (res.status === 201) {
            ...
            setError("something went wrong creating this user", res.data);
          }
        })
        .catch((err) => {
          setError(err);
